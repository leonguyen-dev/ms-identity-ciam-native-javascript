...
| `GRAPH_TIMEOUT_MS` | `1800` | Deadline for the Graph lookup (default 1800). Entra caps the whole callout at ~2s and the phoneMethods endpoint alone can take 1–2.5s; past the deadline the Graph call is aborted and the token is issued **without** the phone claim instead of failing the sign-in. |
