<#
.SYNOPSIS
    Register the OnAttributeCollectionStart custom authentication extension in Entra
    and attach it to a sign-up user flow — via Microsoft Graph.

.DESCRIPTION
    Step 2 of wiring attribute-start-function into Entra. Run after
    ...
    What it does:
      ...
      2. Reuses the SAME authenticationConfiguration.resourceId as the working
         ...
         resource app).
      3. Creates the onAttributeCollectionStartCustomExtension.
      4. PATCHes the chosen user flow's onAttributeCollectionStart handler to point
...
.NOTES
    Schemas verified against Graph v1.0:
      - https://learn.microsoft.com/graph/api/resources/onattributecollectionstartcustomextension
      - https://learn.microsoft.com/graph/api/resources/onattributecollectionstartcustomextensionhandler
...
param(
    ...
    [string]$DisplayName = "ServiceTas - On Attribute Collection Start",
    [string]$Description = "Blocks sign-up for emails/domains on the blocklist (BLOCKED_EMAILS / BLOCKED_DOMAINS) before attribute collection.",
...
# --- 2. Create the OnAttributeCollectionStart custom extension -------------------------------
$body = @{
...
# --- 4. Attach the extension to the flow's onAttributeCollectionStart handler ----------------
$patch = @{
...
Write-Host "`n=========================================================="
Write-Host "Done. OnAttributeCollectionStart extension is wired up." -ForegroundColor Green
...
Write-Host "  <your flow> -> Custom authentication extensions (Before collecting information),"
Write-Host "  or test a sign-up."
Write-Host "=========================================================="
