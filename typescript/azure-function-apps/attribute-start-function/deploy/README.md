# Wiring attribute-start-function into Entra
...
```powershell
cd typescript/azure-function-apps/attribute-start-function/deploy
./1-provision-and-deploy.ps1
```

It creates the app, sets the `BLOCKED_EMAILS`/`BLOCKED_DOMAINS` app settings, publishes
the code, and prints the **Target URL** (including the `?code=` host key) you'll pass to
step 3.
...
```powershell
./2-register-extension.ps1 -TargetUrl "https://<app>.azurewebsites.net/api/attributeCollectionStart?code=<key>"
```

It discovers the OTP extension's `resourceId` automatically, creates the
`onAttributeCollectionStartCustomExtension`, lists your sign-up user flows for you to
pick one, and PATCHes that flow's `onAttributeCollectionStart` handler to point at the
new extension.
...
- Entra admin center → **External Identities → User flows → _your flow_ → Custom
  authentication extensions** — the **Before collecting information** step should list
  this extension.
- Run a sign-up. Watch live results under **Enterprise applications → Sign-in logs →
  ...
  should surface the block message and stop the sign-up after email verification; any
  other email should proceed to attribute collection.
...
- `1003002/1003003` → the function was reached but returned a bad status/body; check
  the Function App logs and the response shape in `attributeCollectionStart.ts`.
